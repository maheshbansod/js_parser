use crate::{
    ...
    parser::{
        atom::Atom,
        expression::{Expression, function_definition::FunctionDefinition},
    },
                ...
                self.tok_look_ahead()
                    .map(|token| {
                        if token.kind == TokenKind::LParen {
                            ...
                            expr.map(|expr| {
                                Term::BracketedExpression(BracketedExpression {
                                    left_paren: token,
                                    right_paren,
...
impl Node for Term {
    fn span(&self) -> crate::tokenizer::Span {
        match self {
            ...
            Term::BracketedExpression(expr) => expr
                .right_paren
                .as_ref()
                .map(|right_paren| Span::concat(expr.left_paren.span, right_paren.span))
                .unwrap_or(expr.left_paren.span),
        }
    }
}
